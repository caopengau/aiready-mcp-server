...
docker-build: ## Build Docker image locally
	...
	docker build -f tooling/docker/Dockerfile.slim -t aiready/cli:latest .
	docker build -f tooling/docker/Dockerfile.slim -t aiready/cli:$$(node -p "require('./packages/cli/package.json').version") .
...
vscode-package: ## Package VS Code extension as VSIX
	...
	cd apps/vscode-extension && pnpm install && pnpm run package
...
action-build: ## Build GitHub Action for Marketplace
	...
	cd tooling/github-action && npm install && npm run build
...
action-publish: ## Instructions for publishing GitHub Action
	...
	@echo "   cp -r tooling/github-action/* /path/to/aiready-action/"
	@echo ""
...
homebrew-test: ## Test Homebrew formula locally
	...
	brew audit --formula tooling/homebrew/aiready.rb || true
